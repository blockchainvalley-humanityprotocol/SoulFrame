...
export default function MintSoulFrame() {
  ...
  const minted = true; // 예시: 이미 발행된 경우
  const metadata = {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#0f172a] via-[#312e81] to-[#a21caf] p-4">
      <div className="backdrop-blur-xl bg-white/10 border border-indigo-200/30 rounded-3xl shadow-2xl flex flex-col items-center px-10 py-12 max-w-md w-full animate-fade-in">
        <h2 className="text-3xl font-extrabold mb-6 bg-gradient-to-r from-indigo-400 via-fuchsia-400 to-cyan-400 bg-clip-text text-transparent drop-shadow-glow">SoulFrame NFT 발행</h2>
        {!hasSBT ? (
          <div className="bg-white/20 border border-red-300/30 rounded-xl px-6 py-4 text-red-400 font-bold mb-3 animate-shake">Humanity 인증이 필요합니다.</div>
        ) : !minted ? (
          <button
            className="w-full bg-gradient-to-r from-indigo-500 via-fuchsia-400 to-cyan-400 text-white py-3 rounded-full font-bold shadow-lg hover:from-fuchsia-400 hover:to-indigo-500 transition text-lg animate-pulse mb-4"
            onClick={() => window.location.href='/mysoulframe'}
          >
            NFT 발행하기
          </button>
        ) : (
