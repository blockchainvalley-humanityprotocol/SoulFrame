...
export default function HumanityAuth() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // 실제 claims 값은 서비스 목적에 맞게 확장
      await issueVC({ claims: { humanity: true }, subject_address: wallet });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#0f172a] via-[#312e81] to-[#a21caf] p-4">
      <div className="backdrop-blur-xl bg-white/10 border border-indigo-200/30 rounded-3xl shadow-2xl flex flex-col items-center px-10 py-12 max-w-md w-full animate-fade-in">
        <h2 className="text-3xl font-extrabold mb-4 bg-gradient-to-r from-indigo-400 via-fuchsia-400 to-cyan-400 bg-clip-text text-transparent drop-shadow-glow">Humanity 인증</h2>
        <div className="mb-4 text-indigo-100 text-center text-base font-medium">
          Humanity Protocol을 통해 1인 1인증을 진행하세요.<br />
          <span className="text-xs text-fuchsia-300">(실제 인증 및 SBT 발급 로직은 API로 연동됨)</span>
        </div>
        <button
        ...
        >
          {loading ? (
            <span className="flex items-center justify-center gap-2"><span className="w-3 h-3 rounded-full border-2 border-white border-t-transparent animate-spin"></span>인증 중...</span>
          ) : (
            <>인증 진행하기</>
          )}
        </button>
        {error && <div className="text-red-400 bg-white/20 border border-red-300/30 rounded px-4 py-2 text-sm font-bold mb-2 animate-shake">{error}</div>}
        <div className="mb-2 mt-2 text-indigo-200 text-xs text-center">인증이 완료되면 SBT 발급 여부가 자동 체크됩니다.</div>
      </div>
      <style>{`
